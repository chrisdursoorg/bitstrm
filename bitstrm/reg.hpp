...
// This bit integer library if for reading byte streams as arrays of
// arbitrary length (as in bit) signed or unsigned integers.  That
// being said types reg::reg and reg::ureg hold that arbitary sized
// value as either an signed or unsigned respectively 'register'
// values
...
namespace bitint { 
  ...
  // split register occurances unless is otherwise sub-optimum
  ////////////////////////////
  // 
  typedef int64_t  reg;    
  //
  ...
  // NOTE that the reg address does NOT reach maximal value (e.g. 2^6
  // addresses [0,64) bits) where the address of 0 bits width is
  // commonly expressed as a zero lenght bitstrm having value of {0}
...
#ifdef USE_ENDIAN
  // this variant allows for ix86 to correctly address sub-register
  // byte memory, prior experiments indicated that this bswap added
  // negligable costs PLATFORM DEPENDENCE
